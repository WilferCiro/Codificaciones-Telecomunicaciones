...
class Gui():
	...
	def ver_puertos(self):
		if self.emisor_serial.is_open and self.receptor_serial.is_open:			
			...
			x = leido2.split('--');		
			self.builder.get_object("DataRec").set_text("Codificated: "+x[0]+"\nError in bit sent: "+x[1]+"\nError in bit received: "+x[2]+"\nData Received: "+x[3]+"Char: "+chr(int(str(x[3]), 2)))
		else:
	...
	def send_text(self, *args):
		if self.emisor_serial.is_open and self.receptor_serial.is_open:
			...
			if len(texto)==1:
				...
				self.builder.get_object("BitsLabel").set_text("Error, word must be 1 characters");	
		else:
	...
	def __init__(self):
		...
		self.builder.get_object("CodificationKind").set_text("Codification: Hamming");
		self.builder.get_object("Title").set_text("Canal Codification: Hamming");
		
		self.builder.get_object("WordSend").set_text("I")
		self.builder.get_object("wordLabel").set_text("Word (1 character)")
